const API = {
  ...
  me: "/admin/api/me",
  hall: (id) => `/admin/api/halls/${id}`,
  editHall: (id) => `/admin/api/halls_edit/${id}`,
};
...
async function loadHall() {
  ...
  if (!Number.isInteger(hallId) || hallId <= 0) {
    setResult("Invalid hall id in URL.", true);
    return false;
  }

  try {
    const response = await fetch(API.hall(hallId), {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
async function submitHall(event) {
  ...
  try {
    const response = await fetch(API.editHall(hallId), {
      method: "PUT",
      headers: {
    ...
    submitBtn.textContent = "Save Hall";
  }
}
...
resetBtn.addEventListener("click", () => {
  if (initialHall) {
    fillFormByHall(initialHall);
    setResult(`Reverted hall #${hallId}`);
  }
});
