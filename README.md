...
Перед первым входом в админку нужно создать учётную запись администратора.  
Сервер при этом **не обязательно** должен быть запущен.

```bash
python create_admin.py
```

Скрипт интерактивно запросит email и пароль:
...
Администратор создан (id=1, email=admin@example.com)
```

После этого можно войти в админку по адресу `/admin/login`.
